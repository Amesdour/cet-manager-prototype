...
INSERT INTO sites (id, name, region, type, capacity, used, status, commune, localisation, accepted_waste) VALUES
  ('CET-JIJ', 'Centre d''Enfouissement Technique Démo',    'Ville Démo', 'CET', 600000, 287400, 'active', 'Démo',     '36.8167° N, 5.7667° E', '["MEN","IND","MED","INE"]'),
  ('CET-TAH', 'Centre d''Enfouissement Technique Site B',    'Site B',             'CET', 400000, 156700, 'active', 'Site B',     '36.7333° N, 5.9000° E', '["MEN","IND","INE"]'),
  ('CET-ELM', 'Centre d''Enfouissement Technique Site C', 'Site C',          'CET', 300000, 198300, 'active', 'Site C',  '36.7500° N, 6.5667° E', '["MEN","IND"]'),
  ('CDI-TAS', 'CDI Site D', 'Site D',          'CDI', 500000,  89000, 'active', 'Site D', '36.6833° N, 6.1333° E', '["INE"]')
...
CREATE TABLE IF NOT EXISTS waste_types (
  ...
  collect_price         DECIMAL(14,2) DEFAULT 0,
  collect_rotation_price DECIMAL(14,2) DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS clients (
  ...
  service_type        VARCHAR(30)    DEFAULT 'treatment_only',
  collect_billing_mode VARCHAR(20)   DEFAULT 'tonnage'
);
...
INSERT INTO clients (id, name, client_type, type, status, credit_enabled, weight_limit_year, credit_limit, consumed, pay_frequency, pay_instrument, phone, address, nif, rc, docs, note, vat_subject) VALUES
  ('C001', 'Commune de Démo',       'state',   'convention', 'approved',     FALSE, 5000, 0,      0, 'monthly', 'cheque', '000 00 00 00',  'Démo Centre',          '000000000000000', '',                 '["Arrêté communal","Convention signée"]',          '', FALSE),
  ('C002', 'Commune Démo B',        'state',   'convention', 'approved',     FALSE, 3000, 0,      0, 'annual',  'bank',   '000 00 00 00',  'Site B',                  '000000000000000', '',                 '["Arrêté communal","Convention signée"]',          '', FALSE),
  ('C003', 'Clinique Médicale Démo',  'private', 'convention', 'approved',     TRUE,  0,    400000, 0, 'monthly', 'bank',   '000 00 00 00',  'Cité Cnep, Démo',       '000000000000000', '18/00-1234567B18', '["RC","NIF","Assurance RC","Bail commercial"]',    '', TRUE),
  ('C004', 'EURL BTP Démo',  'private', 'convention', 'under_review', FALSE, 0,    0,      0, 'monthly', 'cheque', '000 00 00 00',  'Zone Activité, Démo',   '000000000000000', '18/00-7654321B18', '["RC","NIF"]',                                     'Documents reçus, vérification en cours.', TRUE),
  ('C005', 'SPA Démo Algérie',    'private', 'convention', 'pending_docs', FALSE, 0,    0,      0, 'monthly', 'cheque', '000 00 00 00',  'Site C',                '',               '',                 '[]',                                               'En attente de dépôt des documents requis.', TRUE),
  ('C006', 'Client Comptant Démo',       'cash',    'daily',      'approved',     FALSE, 0,    0,      0, '',        '',       '000 00 00 00', 'Démo',                   '',               '',                 '[]',                                               '', FALSE),
  ('C007', 'Entreprise Démo SARL',  'cash',    'daily',      'approved',     FALSE, 0,    0,      0, '',        '',       '000 00 00 00', 'Site B',                   '',               '',                 '[]',                                               '', TRUE),
  ('C008', 'Client Prépayé Démo',        'private', 'prepaid',    'approved',     FALSE, 0,    200000, 0, '',        '',       '000 00 00 00', 'Démo',                   '',               '',                 '[]',                                               'Bonus prépayé 200 000 DA', FALSE)
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO users (id, name, email, password, role, status, phone, matricule, site_id, created_at) VALUES
  ('U001', 'Directeur Administrateur', 'admin@demo-cet.dz',    'admin123', 'admin',    'active',  '000 00 00 00',  'ADM-001',     'all',     '2024-01-15'),
  ('U002', 'Opérateur Démo 1',            'operateur1@demo-cet.dz', 'op1234',   'operator', 'active',  '000 00 00 00', 'OP-2024-001', 'CET-JIJ', '2024-03-10'),
  ('U003', 'Opérateur Démo 2',             'operateur2@demo-cet.dz', 'op1234',   'operator', 'active',  '000 00 00 00', 'OP-2024-002', 'CET-TAH', '2024-03-10'),
  ('U004', 'Opérateur Démo 3',            'operateur3@demo-cet.dz',  'op1234',   'operator', 'pending', '000 00 00 00', 'OP-2024-003', 'CET-ELM', '2024-04-20')
ON CONFLICT (id) DO NOTHING;
